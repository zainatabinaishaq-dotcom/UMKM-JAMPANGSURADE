  ...
  async function handleCheckout(e) {
    ...
    try {
      ...
      for (const item of cart) {
        ...
        if (form.shippingType === "pickup" && form.paymentMethod === "cash") { statusPembayaran = "tunai"; statusPesanan = "pesanan_masuk"; }
  ...
  return (
    <div className="modal-overlay" onClick={onClose}><div className="modal" style={{ maxWidth: 600 }} onClick={(e) => e.stopPropagation()}>
      ...
      <form onSubmit={handleCheckout}><div className="modal-body" style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <div className="form-group"><label>Metode Pengiriman</label><select className="form-input" value={form.shippingType} onChange={(e) => setForm({ ...form, shippingType: e.target.value, paymentMethod: e.target.value === "pickup" ? form.paymentMethod : "transfer" })}><option value="pickup">Ambil di Tempat (Gratis)</option><option value="same_day">Same Day Lokal</option><option value="jne">JNE</option><option value="pos">POS</option><option value="tiki">TIKI</option><option value="jnt">J&T</option><option value="sicepat">SiCepat</option></select></div>
        {form.shippingType === "pickup" && <div className="form-group"><label>Metode Pembayaran</label><select className="form-input" value={form.paymentMethod} onChange={(e) => setForm({ ...form, paymentMethod: e.target.value })}><option value="transfer">Transfer ke Rekening Admin</option><option value="cash">Tunai Saat Ambil Barang</option></select><div style={{ fontSize: 12, color: "var(--text3)", marginTop: 6 }}>Setelah order, pembeli melihat link lokasi toko dan instruksi segera ambil pesanan.</div></div>}
        {form.shippingType === "same_day" && <div className="form-group"><label>Link Google Maps Lokasi Anda</label><input className="form-input" placeholder="Tempel link Google Maps alamat pengiriman" value={form.buyerMapsLink} onChange={(e) => setForm({ ...form, buyerMapsLink: e.target.value })} required /><div style={{ fontSize: 12, color: "var(--orange)", marginTop: 6 }}>Penjual akan menghitung ongkir. Pembayaran aktif setelah ongkir masuk.</div></div>}
        {needsAddress && <div style={{ background: "#FFF8E1", padding: 12, borderRadius: 8 }}><div style={{ fontWeight: 700, marginBottom: 8 }}>Alamat untuk cek ongkir</div><div style={{ fontSize: 12, color: "var(--text3)", marginBottom: 8 }}>Alamat ekspedisi akan otomatis disimpan ke profil dan terisi saat belanja berikutnya.</div><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}><input className="form-input" placeholder="Desa" value={form.buyerVillage} onChange={(e) => setForm({ ...form, buyerVillage: e.target.value })} required /><input className="form-input" placeholder="Kecamatan" value={form.buyerDistrict} onChange={(e) => setForm({ ...form, buyerDistrict: e.target.value })} required /><input className="form-input" placeholder="Kabupaten" value={form.buyerRegency} onChange={(e) => setForm({ ...form, buyerRegency: e.target.value })} required /></div><a href="https://rajaongkir.com/cek-ongkir" target="_blank" rel="noreferrer" style={{ display: "inline-block", marginTop: 10, color: "var(--orange)", fontWeight: 700 }}>Buka RajaOngkir untuk cek ongkir</a></div>}
      </div><div className="modal-footer"><button type="button" className="btn-ghost" onClick={onClose}>Batal</button><button type="submit" className="btn-primary" disabled={loading}>{loading ? "Memproses..." : "Buat Pesanan"}</button></div></form>
    </div></div>
  );
}
...
function BuyerOrderCard({ order, createNotif }) {
  ...
  return (
    <div className="card" style={{ marginBottom: 14 }}>
      ...
      {order.statusPesanan === "menunggu_pembayaran" && order.statusPembayaran !== "menunggu_ongkir" && !order.proofSubmitted && !order.paymentProofUrl && order.paymentMethod !== "cash" && <div style={{ marginTop: 14, paddingTop: 14, borderTop: "1px solid var(--border)" }}><div style={{ fontSize: 13, fontWeight: 600, marginBottom: 8 }}>Upload Bukti Pembayaran</div><div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}><input type="file" accept="image/jpeg,image/png,image/webp" onChange={(e) => { if (e.target.files[0]?.size > 1024*1024) { alert("Maks 1MB"); return; } setFile(e.target.files[0]); }} style={{ fontSize: 13, flex: 1 }} /><button className="btn-primary btn-sm" onClick={uploadProof} disabled={uploadLoading}>{uploadLoading ? "Mengirim..." : "Kirim Bukti"}</button></div></div>}
...
function SellerOrders({ orders, createNotif }) {
  ...
  async function quoteShipping(o) {
    ...
    await updateDoc(doc(db, "orders", o.id), { shippingCost: cost, totalAmount, sellerAmount, pendingShippingQuote: false, statusPembayaran: "menunggu_pembayaran", statusPesanan: "menunggu_pembayaran", courierService: `Ongkir: ${rupiah(cost)}`, updatedAt: serverTimestamp() });
