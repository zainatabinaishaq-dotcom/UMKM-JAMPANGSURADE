    ...
    try {
      ...
      await runTransaction(db, async (transaction) => {
        ...
        for (const bill of payableBills) {
          ...
          if (!freshBill.exists() || ["menunggu_approval", "approved", "auto_paid", "paid"].includes(data.status) || !isOpenCommissionBill({ id: bill.id, ...data })) continue;
...
function ChatCenter({ user, profile, createNotif, mode = "buyer", users = [] }) {
  ...
  async function sendMessage(e) {
    ...
    setBusy(true);
    try {
      const receiverId = activeChat.chatType === "admin_seller"
        ? ((profile?.role === "admin" || profile?.role === "sub_admin") ? activeChat.sellerId : activeChat.adminId)
        : activeChat.participants?.find((id) => id !== user.uid);
