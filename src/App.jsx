...
/* ─── SELLER DASHBOARD ───────────────────────── */
function SellerDashboard({ user, profile, products, orders, wallets, commissionBills = [], paymentSetting, commissionSetting, createNotif, onLogout }) {
  const [tab, setTab] = useState("beranda");
...
function AddProduct({ user, profile, products, hasCommissionDebt = false, commissionDebt = 0, commissionSetting, createNotif }) {
  ...
  async function submit(e) {
    ...
    try {
      ...
      const stockNumber = Number(String(form.stock || "").replace(/\D/g, ""));
      const weightNumber = Number(String(form.weightGram || "1000").replace(/\D/g, "")) || 1000;
      const ref = await addDoc(collection(db, "products"), {
        ...
        price: priceNumber, stock: stockNumber, description: form.description,
        weightGram: weightNumber, sellerAddress: form.sellerAddress,
      ...
      try {
        ...
        console.error("Gagal membuat notifikasi produk baru:", notifError);
      }
      ...
      alert(error?.message || "Gagal upload produk. Coba lagi.");
    } finally {
      setLoading(false);
    }
  }
