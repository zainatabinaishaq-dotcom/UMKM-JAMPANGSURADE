...
function ChatCenter({ user, profile, createNotif, mode = "buyer", users = [] }) {
  ...
  function isAdminSellerChat(chat) {
    return chat?.chatType === "admin_seller"
      || chat?.type === "seller_admin"
      || (Boolean(chat?.adminId) && Boolean(chat?.sellerId) && Array.isArray(chat?.participants));
  }
  ...
  async function sendMessage(e) {
    ...
    setBusy(true);
    try {
      const receiverId = isAdminSellerChat(activeChat)
        ? (isAdminRole ? activeChat.sellerId : activeChat.adminId)
      ...
      if (receiverId && createNotif) {
        const receiverRole = isAdminSellerChat(activeChat)
