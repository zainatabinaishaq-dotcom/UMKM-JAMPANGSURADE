...
<svg xmlns="http://www.w3.org/2000/svg" width="320" height="320" viewBox="0 0 320 320">
  <rect width="320" height="320" rx="28" fill="%23fff7ed"/>
  <circle cx="160" cy="126" r="48" fill="%23fed7aa"/>
  <path d="M72 244c20-54 54-82 88-82s68 28 88 82" fill="%23fdba74"/>
  <text x="160" y="286" font-family="Arial" font-size="18" font-weight="700" fill="%239a3412" text-anchor="middle">UMKM Digital</text>
</svg>`);
...
function getLocationOptions(products = [], filter = emptyLocationFilter) {
  ...
  products.forEach((product) => {
    ...
    if ((!filter.kabupaten || normalizeLocationKey(kab) === normalizeLocationKey(filter.kabupaten)) && kec) kecamatan.add(kec);
    if ((!filter.kecamatan || normalizeLocationKey(kec) === normalizeLocationKey(filter.kecamatan)) && des) desa.add(des);
  });
...
export default function App() {
  ...
  useEffect(() => {
    const validIds = new Set(cart.map((item) => item.id).filter(Boolean));
    setSelectedCartIds((prev) => prev.filter((id, index, arr) => validIds.has(id) && arr.indexOf(id) === index));
  }, [cart]);

  useEffect(() => {
    const hasOverlay = showCart || showCheckout || Boolean(selectedProduct);
    if (!hasOverlay || typeof window === "undefined") return;
    try { window.history.pushState({ umkmOverlay: true }, ""); } catch {}
    const handleBack = () => {
  ...
  function toggleSelectAllCart() {
    const cartIds = [...new Set(cart.map((item) => item.id).filter(Boolean))];
    setSelectedCartIds((prev) => prev.length === cartIds.length ? [] : cartIds);
  }
  ...
  const isAllCartSelected = cart.length > 0 && cart.every((item) => selectedCartIds.includes(item.id));
                  ...
                  {cart.map((item) => {
                    ...
                    return (
                      <div key={item.id} className={`cart-item ${isSelected ? "cart-item-selected" : ""}`}>
                        ...
                        <img src={safeProductImage(item.imageUrl)} onError={handleImageFallback} alt={item.productName || "Produk"} className="cart-item-img" />
...
function ProductCard({ product, onClick, onAddToCart, user }) {
  return (
    <div className="product-card" onClick={onClick}>
      <img src={safeProductImage(product.imageUrl)} onError={handleImageFallback} alt={product.productName || "Produk"} className="product-img" />
      ...
      <div className="modal" style={{ maxWidth: 640 }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          <div style={{ display: "flex", gap: 20, flexWrap: "wrap" }}>
            <img src={safeProductImage(product.imageUrl)} onError={handleImageFallback} alt={product.productName || "Produk"}
    ...
    <div className="modal-overlay" onClick={onClose}><div className="modal" style={{ maxWidth: 600 }} onClick={(e) => e.stopPropagation()}>
      ...
      <form onSubmit={handleCheckout}><div className="modal-body checkout-body">
        ...
        <div className="checkout-summary-card">
          ...
          {cart.map((item) => <div key={item.id} className="checkout-item-row"><img src={safeProductImage(item.imageUrl)} onError={handleImageFallback} alt={item.productName || "Produk"} /><div className="checkout-item-info"><div>{item.productName}</div><span>{rupiah(item.price)} × {item.quantity}</span></div><strong>{rupiah(item.price * item.quantity)}</strong></div>)}
          <div className="divider" />
      ...
      {deletableFiltered.length > 0 && (
        <div className="card" style={{ marginBottom: 14, padding: 12, display: "flex", gap: 10, alignItems: "center", justifyContent: "space-between", flexWrap: "wrap" }}>
          <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, fontWeight: 700 }}>
            <input type="checkbox" checked={deletableFiltered.length > 0 && deletableFiltered.every((o) => selectedOrderIds.includes(o.id))} onChange={toggleSelectAllDeletable} />
            Pilih semua order selesai/dibatalkan
          </label>
  ...
  return (
    <div>
      ...
      {deletableOrders.length > 0 && (
        <div className="card" style={{ marginBottom: 14, padding: 12, display: "flex", gap: 10, alignItems: "center", justifyContent: "space-between", flexWrap: "wrap" }}>
          <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, fontWeight: 700 }}>
            <input type="checkbox" checked={deletableOrders.length > 0 && deletableOrders.every((o) => selectedOrderIds.includes(o.id))} onChange={toggleSelectAllDeletable} />
            Pilih semua order selesai/dibatalkan
          </label>
          <button className="btn-ghost btn-sm" style={{ color: "#EF4444", borderColor: "#FCA5A5" }} onClick={deleteSelectedOrders} disabled={selectedDeletable.length === 0}>Hapus yang Dipilih ({selectedDeletable.length})</button>
        </div>
